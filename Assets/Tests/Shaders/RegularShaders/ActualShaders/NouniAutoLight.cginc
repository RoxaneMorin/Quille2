...
#ifndef NOUNI_AUTOLIGHT_INCLUDED
#define NOUNI_AUTOLIGHT_INCLUDED

#include "HLSLSupport.cginc"
#include "LocalUnityShadowLibrary.cginc"
...
#   define UNITY_LIGHT_ATTENUATION(shadow, atten, input, worldPos) \
...
#   define UNITY_LIGHT_ATTENUATION(shadow, atten, input, worldPos) \
        DECLARE_LIGHT_COORD(input, worldPos); \
        fixed shadow = UNITY_SHADOW_ATTENUATION(input, worldPos); \
        fixed atten = tex2D(_LightTextureB0, dot(lightCoord, lightCoord).rr).r * texCUBE(_LightTexture0, lightCoord).w;
#endif
...
#   define UNITY_LIGHT_ATTENUATION(shadow, atten, input, worldPos) \
        DECLARE_LIGHT_COORD(input, worldPos); \
        fixed shadow = UNITY_SHADOW_ATTENUATION(input, worldPos); \
        fixed atten = tex2D(_LightTexture0, lightCoord).w;
#endif
