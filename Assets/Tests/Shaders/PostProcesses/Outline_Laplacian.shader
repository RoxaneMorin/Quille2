Shader "Hidden/Outline_Laplacian"
{
    ...
    {   
        ...
        {
            edge = laplacian * _OutlineIntensity;
            if (_UseDepthThreshold == 1)
            {
                float thresholdedDepth = _DepthThreshold * (1 - originalDepth) * _DepthThresholdMultiplier;
                edge = (edge > thresholdedDepth) ? edge : 0;
            }
