...
// ------------------------------------------------------------------
    ...
    // Sampler to fetch the light's depth maps by themselves;
    //SamplerState linear_clamp_sampler_ShadowMapTexture;
#else
...
half UnitySampleShadowmap(float3 vec)
{
        ...
        #if defined (SHADOWS_CUBE_IN_DEPTH_TEX)
            // Fetch the depth values by themselves.
            //shadowVals.x = _ShadowMapTexture.Sample(linear_clamp_sampler_ShadowMapTexture, vec+float3( z, z, z));
            //shadowVals.y = _ShadowMapTexture.Sample(linear_clamp_sampler_ShadowMapTexture, vec+float3(-z,-z, z));
            //shadowVals.z = _ShadowMapTexture.Sample(linear_clamp_sampler_ShadowMapTexture, vec+float3(-z, z,-z));
            //shadowVals.w = _ShadowMapTexture.Sample(linear_clamp_sampler_ShadowMapTexture, vec+float3( z,-z,-z));
