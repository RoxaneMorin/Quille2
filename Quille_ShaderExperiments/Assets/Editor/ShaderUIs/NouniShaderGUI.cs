...
public class NouniShaderGUI : ShaderGUI
{
    // ENUMS
    enum WorkflowMode { Metallic, Specular }
    ...
    void DoWorkflowMode()
    {
        ...
        if (IsKeywordEnabled("_SPECULAR_SETUP"))
        {
            workflowMode = WorkflowMode.Specular;
        }

        EditorGUI.BeginChangeCheck();
        workflowMode = (WorkflowMode)EditorGUILayout.EnumPopup(MakeLabel("Workflow Mode"), workflowMode);
        if (EditorGUI.EndChangeCheck())
        {
            RecordAction("Workflow Mode");
            SetKeyword("_SPECULAR_SETUP", workflowMode == WorkflowMode.Specular);
        }
    }
    ...
    void DoMetalness()
    {
        MaterialProperty metalGlossMap = FindProperty("_MetallicGlossMap");
        Texture texture = metalGlossMap.textureValue;
        EditorGUI.BeginChangeCheck();
        editor.TexturePropertySingleLine(MakeLabel(metalGlossMap, "Metallicness (R) and Glossiness (A)"), metalGlossMap, FindProperty("_Metallic"));
        if (EditorGUI.EndChangeCheck() && texture != metalGlossMap.textureValue)
        {
            SetKeyword("_METALLIC_MAP", metalGlossMap.textureValue);
        }
    }
    void DoGlossiness()
    {
    ...
    void DoOthers()
    {
        ...
        //editor.ShaderProperty(FindProperty("_Ramp"), MakeLabel("Toon Ramp"));
